...
public class FieldValidator {
    ...
    private static final Pattern DL_LONG_FORMAT = Pattern.compile(
            "^[A-Z]{2}[-\\s]?[0-9]{2,3}[-\\s]?[0-9]{4}[-\\s]?[0-9]{7}$");
    ...
    public List<String> validateAadhaar(String num) {
        ...
        if (isBlank(num)) {
            errors.add("Aadhaar number not found");
            return errors;
        }
        ...
        } else if (!RegexUtility.isValidAadhaar(digits)) {
            errors.add("Invalid Aadhaar checksum. Got: " + num);
        }
        return errors;
    }
    ...
    public List<String> validatePan(String num) {
        List<String> errors = new ArrayList<>();
        if (isBlank(num)) {
            errors.add("PAN number not found");
            return errors;
        }
    ...
    public List<String> validateDob(String dob) {
        List<String> errors = new ArrayList<>();
        if (isBlank(dob)) {
            errors.add("Date of birth not found");
            return errors;
        }
        if (!isValidDate(dob)) {
            errors.add("Invalid DOB format: " + dob + ". Expected DD/MM/YYYY.");
        }
        return errors;
    }
    ...
    public List<String> validateDate(String date, String fieldName) {
        ...
        if (!isValidDate(date)) {
            errors.add("Invalid " + fieldName + " date: " + date);
        }
        return errors;
    }
    ...
    public List<String> validateName(String name, String label) {
        ...
        if (isBlank(name)) {
            ...
            errors.add(label + " too short: " + name);
        }
        return errors;
    }
